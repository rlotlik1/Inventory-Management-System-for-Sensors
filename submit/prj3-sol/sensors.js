...
{
  const dbFilter=toDbUser(filter,false);
  const dbTable=this.db.collection(COLLECTIONS);
  const ret= dbTable.find(dbFilter);
  const retUsers=ret.toArray();
  return fromDbUsers(retUsers);
}
...
function toDbUser(user, mustHaveId=true) {
  if (user._id) {
  ...
  if (mustHaveId && !dbUser.id) {
    throw new UserError('NO_ID', `no id for user ${JSON.stringify(user)}`);
  }
